...
class DES_ascii {
    ...
    public static String bits2ascii(boolean bits[]){
      int num_chars = bits.length/8;
      String ascii_parts = "";
      for (int i = 0; i < num_chars; i++) {
        int temp = bits2int(Arrays.copyOfRange(bits, i*8, (i+1)*8));
        ascii_parts += Character.toString ((char) temp);
      }
        return ascii_parts;
    }
    ...
    public static boolean[] asciistr2bits(String s){
        int num_chars = s.length();
        boolean[] bits = new boolean[num_chars*8];
        boolean[] temp;
        for (int i = 0; i < num_chars; i++) {
            temp = int2bits((int)s.charAt(i), 8);
            for (int j=0;j<8;j++){
            bits[i*8+j] = temp[j];
            }
        }
    ...
    public static boolean[][] keyGen(String init_key) {
      boolean[] K = asciistr2bits(init_key);
    ...
    public static String DESencrypt(String hexMessage, boolean[][] keys) {
      ...
      boolean[] M = asciistr2bits(hexMessage);
      ...
      return bits2ascii(permute(concat(R[16], L[16]),IP_inv_matrix));
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter ASCII key (8 chars): ");
        strKey = scanner.nextLine();
        Keys = keyGen(strKey);
        System.out.println("Enter Message (16 hex chars): ");
        message = scanner.nextLine();

        int extra = message.length()%8;
        if (extra!=0){
          String pad = "#";
          String repeated = new String(new char[8-extra]).replace("\0", pad);
          message = message + repeated;
        }
        System.out.println("Message : "+message);
        String[] message_parts = message.split("(?<=\\G.{8})");
        String enc_string = "";
        System.out.println(Arrays.toString(message_parts));
        for (int i = 0;i<message_parts.length;i++){
          enc_string += DESencrypt(message_parts[i], Keys);
        }
        System.out.println("Encrypted : "+enc_string);

        // Reverse order of Keys for decryption
        Collections.reverse(Arrays.asList(Keys));

        String[] enc_message_parts = enc_string.split("(?<=\\G.{8})");
        String dec_string = "";
        for (int i = 0;i<enc_message_parts.length;i++){
          dec_string += DESencrypt(enc_message_parts[i], Keys);
        }
        System.out.println("Decrypted : "+dec_string);
        scanner.close();
